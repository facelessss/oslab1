// Simple command-line kernel monitor useful for
	...
	{ "time", "Display the running time(in clocks cycles) of a command(in clocks cycles)" , mon_time },
	};
	...
	void (*overflow)() = do_overflow;
	uint32_t overflow_addr = (uint32_t)overflow + 3 ;
	int i = 0;
	for(i = 0;i < 4;i++){
	//*(pret_addr+i) = (overflow_addr>>(8*i))&0xff;
	memset(str, 0, 256);
	memset(str, 0xd, (overflow_addr >> (8*i)) & 0xff);
	str[((overflow_addr >> (8*i)) & 0xff)] = '\0';
	cprintf("%s%n\n", str, pret_addr + i);
	}
	...
	debuginfo.eip_fn_name,eip - debuginfo.eip_fn_addr);
