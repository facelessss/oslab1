// Stripped-down primitive printf-style formatting routines,
	...
	*/
	...
	//I think moditying this function is not a smart choice
	//so I add a new function
	...
	int sp = 0;
	if (num >= base) {
	sp = printnumRP(putch, putdat, num / base, base, width - 1, padc, or + 1);
	} else {
		sp = width;
	}
	putch("0123456789abcdef"[num % base], putdat);
	...
	int base, lflag, width, precision, altflag,pflag,rtpflag;
	char padc;
	...
	number:
	if(rtpflag == 1){
	int forRP = 0;//int nous = 0;
	printnumRP(putch, putdat, num, base, width, padc,  forRP);
	}else{
